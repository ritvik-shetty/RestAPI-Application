...
logging.basicConfig(level=logging.DEBUG, filename="logs.log",filemode='w' , format='%(asctime)s,%(name)s:%(levelname)s:%(message)s')
...
def posts():
    if request.method == 'GET':
        app.logger.info("User requested a GET request")
        try:
...
def delete_emp_route(emp_id):
    app.logger.info("User requested a DELETE request")
    try:
        connection = sqlite3.connect('empdatabase.db')
        cursor = connection.cursor()
        cursor.execute('SELECT * FROM emp WHERE id = ?', (emp_id,))
